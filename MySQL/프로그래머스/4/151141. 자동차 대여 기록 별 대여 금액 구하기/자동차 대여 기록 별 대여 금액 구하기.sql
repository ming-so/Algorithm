-- 코드를 입력하세요
SELECT HISTORY_ID, MIN(ROUND(DAILY_FEE * (100 - IFNULL(DISCOUNT_RATE, 0)) / 100 * (DATEDIFF(END_DATE, START_DATE) + 1))) FEE
FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY HIS 
     ...
     LEFT JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN DIS 
        ON CAR.CAR_TYPE = DIS.CAR_TYPE AND 
           DATEDIFF(END_DATE, START_DATE) + 1 >= REGEXP_REPLACE(DURATION_TYPE, '[^0-9]', '')
